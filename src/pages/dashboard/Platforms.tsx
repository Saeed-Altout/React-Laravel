...
export default function Platforms() {
  ...
  const { data } = useFetch({ endpoint: "platform/index" });
  ...
  return (
    <div className="flex-1 space-y-5">
      <CreateModal
        endpoint="platform"
        messageError="Failed added platform!"
        messageSuccess="Platform added"
        isOpen={isOpen}
...
interface PlatformsColumn {
  id: number | string;
  name: string;
  icon: string;
}

const columns: ColumnDef<PlatformsColumn>[] = [
  {
    accessorKey: "name",
    header: "Name",
  },
  ...
  },
  {
    id: "actions",
    cell: ({ row }) => (
      <CellAction initialData={row.original} endpoint="platform" />
    ),
  },
];
